...
export default function ShopDetailPage() {
  ...
  const shopProducts = useMemo(
    () => (shop ? products.filter((p) => p.shopId === shop.id) : []),
    [shop, products],
  )
  ...
  const totals = useMemo(() => {
    ...
    for (const p of shopProducts) counts[stockStatus(p.stock).key] += 1
    return {
      products: shopProducts.length,
      stock: totalStock(shopProducts),
      value: inventoryValue(shopProducts),
      counts,
    }
  }, [shopProducts])
      ...
      <Card className="gap-0 overflow-hidden py-0">
        <div className="flex flex-col gap-3 border-b p-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {filtered.length} of {shopProducts.length} shown
            </p>
          </div>
